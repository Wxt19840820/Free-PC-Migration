<#
.SYNOPSIS
    PC-Migrator v1.0 — One-click Windows PC Migration
.DESCRIPTION
    Give it admin credentials, it handles everything:
    - Network auto-config (SMB + WinRM + Firewall + UAC bypass)
    - Full-drive migration (robocopy with optimal WiFi params)
    - Dev software migration (Python/PostgreSQL/GeoServer/QGIS/Anaconda)
    - Configuration sync (pip/git/PATH/Claude Code skills/env vars)
    - 34 automated pre-flight checks & fixes for known pitfalls

    Born from migrating 200GB over WiFi with every Windows quirk
    documented and automated.

.PARAMETER TargetComputer
    Target machine hostname or IP address.
.PARAMETER TargetUser
    Target admin username. For local accounts, use format:
    "COMPUTERNAME\UserName" or ".\UserName"
.PARAMETER TargetPass
    Target admin password.
.PARAMETER SourceDrives
    Comma-separated list of drive letters to migrate (default: "D,E").
.PARAMETER BackupDir
    Target backup directory (default: "C:\backup").
.PARAMETER RoboThreads
    Robocopy thread count. 16 is optimal for WiFi (default: 16).
.PARAMETER SkipSoftware
    Skip dev software migration.
.PARAMETER DryRun
    Run only pre-flight checks without actual data transfer.
.EXAMPLE
    .\PC-Migrator.ps1 -TargetComputer "DESKTOP-TARGET" -TargetUser ".\Admin" -TargetPass "P@ssword!"

    Migrates D and E drives to target, including dev software.

.EXAMPLE
    .\PC-Migrator.ps1 -TargetComputer "10.99.72.6" -TargetUser "10.99.72.6\Admin" -TargetPass "pass" -DryRun

    Runs all pre-flight checks and diagnostics only.

.NOTES
    Requirements:
    - Administrator privileges on THIS machine
    - Administrator credentials on TARGET machine
...
function Write-Pass    { $script:CheckResults.Add(@{Status="PASS";  Item="$args"}) | Out-Null; Write-Host "    [PASS] $args" -ForegroundColor Green }
function Write-Fix     { $script:CheckResults.Add(@{Status="FIXED"; Item="$args"}) | Out-Null; Write-Host "    [FIX]  $args" -ForegroundColor Yellow }
function Write-Fail    { $script:CheckResults.Add(@{Status="FAIL";  Item="$args"}) | Out-Null; Write-Host "    [FAIL] $args" -ForegroundColor Red }
function Write-Manual  { $script:CheckResults.Add(@{Status="MANUAL";Item="$args"}) | Out-Null; Write-Host "    [MANUAL] $args" -ForegroundColor Magenta }
...
function Get-NetworkProfile {
    ...
    if ($isWiFi) {
        Write-Info "Network: WiFi detected ($($adapter.Name)) — using WiFi-optimized params"
        return @{
            Type        = "WiFi"
        ...
        Write-Info "Network: Ethernet detected ($($adapter.Name)) — using maximum throughput params"
        return @{
            Type        = "Ethernet"
        ...
        Write-Info "Network: Unknown type — using conservative params"
        return @{
            Type        = "Unknown"
            Threads     = 8
            IPG         = 5
...
function Export-WingetList {
    ...
    try {
        ...
        Write-Info "Winget found — exporting installed packages"
        ...
        Write-Info "Winget not available — skipping (Windows 10/11 has it built-in)"
    }
}
...
function Export-WSLDistributions {
    ...
    try {
        ...
        Write-Info "WSL not available — skipping"
    }
}
...
╔══════════════════════════════════════════════╗
║   PC-Migrator v1.0 — Pre-Flight Health Check ║
║   $(' ' * 20)Source: $($script:SourceComputer) ($script:SourceIP)
║   $(' ' * 20)Target: $script:TargetHost ($script:TargetIP)
╚══════════════════════════════════════════════╝

"@ -ForegroundColor Cyan

    Write-Host "Running 34 automated checks covering all known Windows
migration pitfalls. Each check marked [PASS]/[FIX]/[MANUAL].
" -ForegroundColor Gray
    ...
    else { Write-Fail "A1: NOT running as Administrator — re-run PowerShell as Admin" }
    ...
    if ($netProfile.NetworkCategory -eq "Private") {
        ...
        Write-Fix "A2: Network was $($netProfile.NetworkCategory) — changing to Private"
    ...
    foreach ($d in $SourceDrives) {
        ...
        if (Test-Path $path) {
            ...
            Write-Pass "A4: Drive $d found — ${used}GB used"
        } else {
    ...
    else { Write-Fail "A5: PowerShell too old — need 5.1+" }
    ...
    if ($wifiAdapter) {
        ...
        if ($powerMgmt -eq 0) {
            ...
            Write-Manual "A6: WiFi adapter may have power saving enabled — consider disabling via Device Manager > Network Adapters > [your WiFi] > Properties > Power Management > uncheck 'Allow computer to turn off'"
        }
    }
    ...
    if (Test-Connection -ComputerName $script:TargetIP -Count 2 -Quiet) {
        ...
        Write-Fail "B1: Cannot ping $($script:TargetIP) — check network/cables"
    }
    ...
    else { Write-Fix "B2: SMB port 445 closed — will attempt to configure via WinRM or manual step" }
    ...
    else { Write-Fix "B3: WinRM port 5985 closed — will configure on target" }
    ...
    try {
        ...
        if ($script:TargetIP -in $ips) {
            ...
            Write-Fix "B5: DNS mismatch — $($script:TargetHost) resolves to $($ips -join ', ') but target IP is $($script:TargetIP). Will use hostname for SMB."
        }
    ...
    try {
        ...
        Write-Fix "C0: WinRM not available yet — will configure during bootstrap"
    }
    ...
    if ($script:WinRMAvailable) {
        $remoteChecks = {
            ...
            else { $results += "FIX: C1: Target network was $($prof.NetworkCategory) — changing to Private"; Get-NetConnectionProfile | Set-NetConnectionProfile -NetworkCategory Private }
            ...
            else { $results += "FAIL: C6: Target C: only ${free}GB free — insufficient for migration!" }
            ...
            # C8: Firewall — SMB rules (GOTCHA: both English AND Chinese group names)
            $fwSMB = Get-NetFirewallRule -Enabled True -Direction Inbound -ErrorAction SilentlyContinue |
                Where-Object { $_.DisplayName -match "SMB|File and Printer|文件和打印机" }
            if ($fwSMB) {
                ...
                Enable-NetFirewallRule -DisplayGroup "*文件和打印机*" -ErrorAction SilentlyContinue
            ...
            catch { $results += "INFO: C10: Node.js not installed — Claude Code will need it" }

            # C11: Claude Code check
            try { $cv = claude --version 2>$null; $results += "PASS: C11: Claude Code $cv installed" }
            catch { $results += "INFO: C11: Claude Code not installed — will install during setup" }
        ...
        Write-Manual "C1-C11: WinRM unavailable — remote checks skipped. Target checks will run after bootstrap."
    }
    ...
    if ($RoboThreads -gt 16) {
        Write-Info "E: WiFi mode — /MT:$RoboThreads threads, estimated 30-60 min"
    } else {
        Write-Info "E: /MT:$RoboThreads threads + /IPG:5 — WiFi-optimized"
    }
    ...
    if ($DryRun) {
        Write-Host "`nDryRun mode — exiting after checks." -ForegroundColor Yellow
        exit 0
    }
}
...
function Invoke-TargetBootstrap {
    ...
    if ($script:WinRMAvailable) {
        Write-Pass "WinRM already working — bootstrap complete"
        return
    }
    ...
    $bs = @'
# PC-Migrator Bootstrap — Run as Administrator on target machine
...
# 2. Firewall — SMB (GOTCHA: Chinese Windows uses different group names)
netsh advfirewall firewall set rule group="File and Printer Sharing" new enable=Yes >$null 2>&1
Enable-NetFirewallRule -DisplayGroup "*File and Printer Sharing*" -ErrorAction SilentlyContinue
Enable-NetFirewallRule -DisplayGroup "*文件和打印机共享*" -ErrorAction SilentlyContinue
...
"@ -ForegroundColor Yellow
    ...
    try {
        ...
        Write-Warn "WinRM still not available — continuing with SMB-only mode"
    }
}
...
function Start-RobocopySession {
    ...
    if ($ec -lt 8) {
        ...
        Write-Warn "$Label had errors (robocopy exit $ec) — check $logFile"
        return $false
    }
}
...
function Invoke-DataMigration {
    ...
    # User config (settings, agents, projects — NOT sessions/cache)
...
function Invoke-SoftwareExport {
    ...
    } catch { Write-Info "pip not found — skipping" }
    ...
    if ((Test-Path $pgDump) -and $script:DetectedSoftware.Name -contains "PostgreSQL") {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Warn "PostgreSQL dump failed — you may need to dump manually"
        }
    }
...
function Invoke-TargetConfiguration {
    ...
    if (-not $script:WinRMAvailable) {
        Write-Manual "WinRM unavailable — run manual config script on target:"
        $cfg = @"
...
╔══════════════════════════════════════════════╗
║          PC-Migrator — Complete!              ║
╠══════════════════════════════════════════════╣
║ Source:    $($script:SourceComputer.PadRight(30)) ║
║ Target:    $($script:TargetHost.PadRight(30)) ║
║ Duration:  ${elapsed} min $(' ' * (24 - ${elapsed}.ToString().Length))                   ║
║ Backup:    $script:BackupUNC
╚══════════════════════════════════════════════╝

Data on target:
  $BackupDir\D          — Drive D contents
  $BackupDir\E          — Drive E contents
  $BackupDir\C_Users    — User profile data
  $BackupDir\ClaudeConfig\skills      — Claude Code skills
  $BackupDir\ClaudeConfig\user_claude — User settings
  $BackupDir\Software   — Dev tools
  $BackupDir\Exports    — pip/PATH/git configs
...
╔══════════════════════════════════════════════╗
║           PC-Migrator v1.0                    ║
║   One-Click Windows PC Migration              ║
║   github.com/YOUR_USERNAME/PC-Migrator        ║
╚══════════════════════════════════════════════╝
